...
data = {
    'daemonHost': BAZA_COIN_DAEMON_HOST,
    'daemonPort': int(BAZA_COIN_DAEMON_PORT),
    'filename': BAZA_WALLET_FILENAME,
    'password': BAZA_WALLET_PASSWORD
}
...
class BazaAPIWrapper(object):
    ...
    def get_api_response(self, req_method, api_endpoint, data=None):
        url = BAZA_WALLET_API_URL + api_endpoint
        headers = {
            'X-API-KEY': BAZA_WALLET_API_KEY
        }
